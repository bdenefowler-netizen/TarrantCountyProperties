...
def _part_members(uploaded_file):
    ...
    for info in zf.infolist():
        if info.is_dir():
            continue
        match = _PART_RE.search(info.filename)
        if match:
...
def _rows_from_split_parts(parts, targets: set[str]) -> pd.DataFrame:
    ...
    line from one part into the next, so pandas/CSV parsing never sees broken
    records. Only rows sharing a target house number are retained.
    """
        ...
        with zf.open(info, "r") as stream:
            while True:
                ...
                for raw_line in lines:
                    ...
                    if header is None:
                        header = text.split("|")
                        wanted_positions = [(i, col) for i, col in enumerate(header) if col in wanted]
                        try:
                            situs_pos = header.index("Situs_Address")
                        except ValueError as exc:
                            raise ValueError("Split TAD property file does not contain Situs_Address.") from exc
                        continue
                    ...
                    if situs_pos is None or situs_pos >= len(fields):
                        continue
                    if targets:
                        m = re.match(r"^\s*(\d+)", fields[situs_pos] or "")
                        if not m or m.group(1) not in targets:
                            continue
                    row = {col: fields[i] if i < len(fields) else "" for i, col in wanted_positions}
                    rows.append(row)
    ...
    if carry.strip():
        ...
        if header is None:
            ...
            if situs_pos is not None and situs_pos < len(fields):
                ...
                if targets:
                    m = re.match(r"^\s*(\d+)", fields[situs_pos] or "")
                    keep = bool(m and m.group(1) in targets)
                if keep:
                    rows.append({col: fields[i] if i < len(fields) else "" for i, col in wanted_positions})
...
def load_tad_uploads(uploaded_files, target_addresses: list[str], max_mb: int = 200):
    ...
    try:
        for uploaded in uploaded_files:
            ...
            if size_mb > max_mb:
                errors.append(f"{uploaded.name}: upload is {size_mb:.1f} MB; maximum is {max_mb} MB")
                continue
            ...
            if uploaded.name.lower().endswith(".zip"):
                ...
                if found:
                    ...
                    for base, number, info in found:
                        multipart_groups[base].append((number, zf, info, uploaded.name))
                    continue
                if zf is not None:
                    zf.close()
        ...
        for uploaded in regular_files:
            try:
                df, source = load_tad_dataframe(uploaded, target_addresses, max_mb)
                if not df.empty:
                    frames.append(df)
        ...
        for base, members in multipart_groups.items():
            ...
            if numbers != expected:
                ...
                errors.append(f"{base}: missing split part(s) {', '.join(map(str, missing))}")
                continue
            try:
                ...
                sources.append(f"{' + '.join(zip_names)} → {base}.part1..part{numbers[-1]}")
            except Exception as exc:
